...
fs.readFile("AOC_7_input.txt", (err, data) => {
    ...
    const needToCleanUp = Number(UNUSED_SPACE_NEEDED - freeSpace);

    const sortedFolders = folders.sort((a, b) => a.size - b.size);
    let folderToDelete = undefined;
    sortedFolders.every(folder => {
        if (needToCleanUp <= folder.size) {
    ...
    console.log('Best folder to delete: ', folderToDelete);
